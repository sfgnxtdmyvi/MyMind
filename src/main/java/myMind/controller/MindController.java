...
public class MindController {
    ...
    public void addSibling() {
        NodeModel parentModel = selectedNode.getModel().getParent();
        // 基于父结点位置加上偏移，创建子结点
        double childX = parentModel.getX() + 180;
        double childY = parentModel.getY() + (parentModel.getChildren().size() * 50);
        NodeModel childModel = new NodeModel(nextId(), "", childX, childY);
        parentModel.addChild(childModel);
        addNode(childModel);
    }
    ...
    public void refreshLines() {
        ...
        // 遍历所有结点模型，绘制从父到子的连线
        for (MindNode childView : nodeMap.values()) {
            NodeModel childModel = childView.getModel();
            NodeModel parentModel = childModel.getParent();

            if (parentModel != null) {
                MindNode parentNode = nodeMap.get(parentModel.getId());
                if (parentNode != null) {
                    Point2D start = getRightPoint(parentNode);
                    Point2D end = getLeftPoint(childView);
